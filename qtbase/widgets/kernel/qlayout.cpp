...
{
    addChildWidget(w); // make qlayout's parent as w's parent
...
void QLayout::addChildWidget(QWidget *w)
{
	...
	// w is in another widget, delete it in another and move to this
	...
	// transfer parentship to this
    if (!pw && mw)
        w->setParent(mw);
    w->setAttribute(Qt::WA_LaiqdOut);
    if (needShow)
        QMetaObject::invokeMethod(w, "_q_showIfNotHidden", Qt::QueuedConnection); //show later
}
...
{
    QSize s = sizeHint();  //oye,  defiend in QLayoutItem, but QLayout did not impl it, so derived class must do that task
